//REF: https://raw.githubusercontent.com/hawell/fpl/c14ddb33c074f8d7a0efe780b71456b7511b0836/src/clahe.c
/* 
...
void MakeHistogram (uint8* pImage, unsigned int uiXRes,
					unsigned int uiSizeX, unsigned int uiSizeY,
					unsigned long* pulHistogram,
					unsigned int uiNrGreylevels, uint8* pLookupTable);
void MapHistogram (unsigned long* pulHistogram, uint8 Min, uint8 Max,
				   unsigned int uiNrGreylevels, unsigned long ulNrOfPixels);
void MakeLut (uint8 * pLUT, uint8 Min, uint8 Max, unsigned int uiNrBins);
void Interpolate (uint8 * pImage, int uiXRes, unsigned long * pulMapLU,
				  unsigned long * pulMapRU, unsigned long * pulMapLB,  unsigned long * pulMapRB,
				  unsigned int uiXSize, unsigned int uiYSize, uint8 * pLUT);
 
/************************** main function CLAHE ******************/ 
int CLAHE (uint8* pImage, unsigned int uiXRes, unsigned int uiYRes,
		   uint8 Min, uint8 Max, unsigned int uiNrX, unsigned int uiNrY, 
...
/* 
    ...
    uint8* pImPointer;		   /* pointer to image */ 
    uint8 aLUT[uiNR_OF_GREY];	    /* lookup table used for scaling of input image */ 
...
{ 
    int i; 
    const uint8 BinSize = (uint8) (1 + (Max - Min) / uiNrBins); 
...
{ 
    ...
    uint8 GreyValue; unsigned int uiNum = uiXSize*uiYSize; /* Normalization factor */ 
    ...
    if (uiNum & (uiNum - 1))   /* If uiNum is not a power of two, use division */ 
		...
		{ 
			...
			{ 
				...
				*pImage++ = (uint8 ) ((uiYInvCoef * (uiXInvCoef*pulMapLU[GreyValue] 
